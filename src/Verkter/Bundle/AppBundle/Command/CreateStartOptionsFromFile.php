 ...
 */
class CreateStartOptionsFromFile extends Command
{
    ...
    {
        $this->setName('verkter:create:start_attribute_options')
            ->setDescription('Create options')
            ->addArgument('file_path', InputArgument::REQUIRED, 'File path: data/verkter_rows_p.csv');
    }
        ...
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            ...
            while (($data = fgetcsv($handle, 1000, "|")) !== FALSE) {
                if (1000 == $i) {
                    ...
                    $dataCreateOption = [];
                    $dataCreateOption[self::ATTRIBUTE_CODES[1]]['data'] = [
