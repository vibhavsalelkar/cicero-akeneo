...
namespace Webkul\ImageGalleryBundle\Connector\Reader\Import;
...
class GalleryReader implements \FileReaderInterface
{
    ...
    public function read()
    {
        ...
        if(!isset($item['media'])){
            $headers = explode(',', $this->fileIterator->getHeaders()[0]);
            $data = explode(',', $data[0]);
            $item = array_combine($headers, $data);
        }
        $mediavalues =  explode("&", $item['media']);
        
        $code = isset($item['code']) ? $item['code'] : null;
        $item['media'] = $this->connectorService->transform($mediavalues, $this->fileIterator->getDirectoryPath(), $code);
        try {
